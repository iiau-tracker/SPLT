...
import sys
# sys.path.append('/home/masterbin-iiau/lasot_train_V/lib')
'''下面是我本地的slim里与ResNet有关的模块'''
...
'''2019.2.14 这个脚本是基于resnet_v1.py修改而成的
原来的脚本用面向对象的思路编写的,不太适合被其他函数调用
于是我把它改成了面向过程的'''
...
_variables_to_fix = {}
pretrained_model = '/home/masterbin-iiau/train_V/lib/resnet_v1_50.ckpt'
# pretrained_model = '/home/masterbin-iiau/train_V/experiments/previous/VID_N/V_resnet50_VID_N/V_resnet50_VID_N-65624'
'''原来的写法里:block1的stride参数设置为2,我觉得不是很合理,于是改成了1'''
...
def _image_to_feat(image, is_training, reuse=None):
    ...
    '''如果希望下面某个或者某几个残差模块参数固定,则只需要把它们的is_training参数设置成False'''
    ''' Residual Block1'''
    with slim.arg_scope(resnet_arg_scope(is_training= False)):
    ...
    '''最后的结果是一个归一化后的128维的特征向量(与FaceNet的设计相同)'''
...
'''专门把conv1的RGB权重转化成BGR权重(因为我是用OpenCV读的图片)'''
...
def restore_variables(sess):
    ...
    '''tf.variables_initializer--->返回一个对列表中的变量初始化的Op'''
    sess.run(tf.variables_initializer(variables, name='init'))
    '''作者写的从checkpoint文件中读取变量的函数'''
